...
def computeRGBToGreyscale(pixel_array_r, pixel_array_g, pixel_array_b, image_width, image_height):
    ...
    for row in range(image_height):
        for col in range(image_width):
            ...
            greyscale_pixel_array[row][col] = 2*round(red+green+blue)
...
def calculatePercentageWhite(pixel_array, top, bottom, left, right):
    sum = 0
    count = 0
...
def main():
    ...
    threshold = 150
    px_array_r = scaleTo0And255AndQuantize(px_array_r,image_width, image_height)
    px_array_g = scaleTo0And255AndQuantize(px_array_g,image_width, image_height)
    px_array_b = scaleTo0And255AndQuantize(px_array_b,image_width, image_height)
    px_array = computeRGBToGreyscale(px_array_r, px_array_g, px_array_b, image_width, image_height)
    quantized1Image = scaleTo0And255AndQuantize(px_array, image_width, image_height)
    stdDeviationImage = computeStandardDeviationImage5x5(quantized1Image, image_width, image_height)
    quantized2Image = scaleTo0And255AndQuantize(stdDeviationImage, image_width, image_height)
    thresholdImage = computeThresholdGE(quantized2Image, threshold, image_width, image_height)
    for i in range(4):
        dilationImage = computeDilation8Nbh3x3FlatSE(thresholdImage, image_width, image_height)
    for i in range(4):
        erosionImage = computeErosion8Nbh3x3FlatSE(dilationImage, image_width, image_height)
    (px_array_lables, px_size) = computeConnectedComponentLabeling(erosionImage, image_width, image_height)
    closingImage = computeThresholdGE(quantized2Image, 1, image_width, image_height)
        ...
        for y in range(image_height):
            for x in range(image_width):
                if px_array_lables[y][x] == sizeDict[labelToDetect][0]:
                    ...
                    right = max(right, x)
        if (top-bottom != 0):
            if ((right-left)/(top-bottom)) > 1.5 and ((right-left)/(top-bottom)) < 5 and calculatePercentageWhite(closingImage, top, bottom, left, right)>0.4:
                break
    ...
    axs1[0, 0].imshow(stdDeviationImage, cmap='gray')
    axs1[0, 1].set_title('After performing threshold')
    axs1[0, 1].imshow(thresholdImage, cmap='gray')
    axs1[1, 0].set_title('After closing the image')
    axs1[1, 0].imshow(closingImage, cmap='gray')
